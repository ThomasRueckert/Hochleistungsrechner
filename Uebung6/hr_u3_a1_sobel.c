...
const char oclKernelName[] = "KirschFilter";
const char *oclKernelSource[] = {
	"__kernel void sobelFilter(const __global uchar4 *inputImage, __global uchar4 *outputImage)\n"
	"{\n"
	"	float4 G[8];\n"	
	...
	"		float4 i22 = convert_float4(inputImage[pos + 1 + width]);\n"
	"\n"
	"		G[0] = - (float4)(3)*i00 - (float4)(3)*i10 - (float4)(3)*i20 - (float4)(3)*i01 - (float4)(3)*i21 + (float4)(5)*i02 + (float4)(5)*i12 + (float4)(5)*i22;\n"
	"		G[1] = - (float4)(3)*i00 - (float4)(3)*i10 - (float4)(3)*i20 + (float4)(5)*i01 - (float4)(3)*i21 + (float4)(5)*i02 + (float4)(5)*i12 - (float4)(3)*i22;\n"
	"		G[2] =   (float4)(5)*i00 - (float4)(3)*i10 - (float4)(3)*i20 + (float4)(5)*i01 - (float4)(3)*i21 + (float4)(5)*i02 - (float4)(3)*i12 - (float4)(3)*i22;\n"
	"		G[3] =   (float4)(5)*i00 + (float4)(5)*i10 - (float4)(3)*i20 + (float4)(5)*i01 - (float4)(3)*i21 - (float4)(3)*i02 - (float4)(3)*i12 - (float4)(3)*i22;\n"
	"		G[4] =   (float4)(5)*i00 + (float4)(5)*i10 + (float4)(5)*i20 - (float4)(3)*i01 - (float4)(3)*i21 - (float4)(3)*i02 - (float4)(3)*i12 - (float4)(3)*i22;\n"
	"		G[5] = - (float4)(3)*i00 + (float4)(5)*i10 + (float4)(5)*i20 - (float4)(3)*i01 + (float4)(5)*i21 - (float4)(3)*i02 - (float4)(3)*i12 - (float4)(3)*i22;\n"
	"		G[6] = - (float4)(3)*i00 - (float4)(3)*i10 + (float4)(5)*i20 - (float4)(3)*i01 + (float4)(5)*i21 - (float4)(3)*i02 - (float4)(3)*i12 + (float4)(5)*i22;\n"
	"		G[7] = - (float4)(3)*i00 - (float4)(3)*i10 - (float4)(3)*i20 - (float4)(3)*i01 + (float4)(5)*i21 - (float4)(3)*i02 + (float4)(5)*i12 + (float4)(5)*i22;\n"
	"\n"
	"		int i;\n"
	"		float4 max;\n"
	"		for (i = 0; i < 8; i++) {\n"
	...
	"			}\n"
	"		}\n"
	"		outputImage[pos] = convert_uchar4(max);\n"
	"	}\n"
	"\n"
	"}\n"
};
